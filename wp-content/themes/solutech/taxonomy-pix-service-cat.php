<?php 
/*** The taxonomy for services category. ***/

$pix_layout = pixtheme_get_layout('service-cat');
$pix_width_class = pixtheme_get_layout_width( 'service-cat' );

$pix_term = get_term_by( 'slug', get_query_var( 'term' ), get_query_var( 'taxonomy' ) );
$services_page = '';//pixtheme_get_setting('pix-page-service-cat', '');
$all_services = '';
if ( '' != $services_page ) {
...
$pix_col = pixtheme_get_setting('pix-col-service-cat', '3');
$pix_gap = '50';

$image_size = $image_class = pixtheme_get_setting('pix-images-service-cat', '');
if($pix_col == 3){
    $image_size .= '-col-3';
} elseif($pix_col > 3) {
    $image_size .= '-col-4';
...
<section class="blog pix-taxonomy">
    <div class="container <?php echo esc_attr($pix_width_class)?>">
        <div class="row">
			...
			<?php
                $pix_category_description = get_term_field( 'description', $pix_term->term_id, 'pix-service-cat' );
				if( !is_wp_error( $pix_category_description ) && $pix_category_description != '' ) :
            ?>
					<div class="section-heading text-center">
						<div class="section-subtitle"><?php echo wp_kses_post($pix_category_description);?></div>
					</div>

			<?php
				endif;

                $pix_categories = get_objects_in_term( $pix_term->term_id, 'pix-service-cat');
                ...
                <?php
                    while ( $wp_query->have_posts() ) :
                        ...
                        $pix_linked_list_cats = pixtheme_get_post_terms( array( 'taxonomy' => 'pix-service-cat', 'items_wrap' => '%s' ) );
                ?>
