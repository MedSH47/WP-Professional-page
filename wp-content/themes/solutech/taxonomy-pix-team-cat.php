...
$pix_layout = pixtheme_get_layout('team-cat');
$pix_width_class = pixtheme_get_layout_width( 'team-cat' );

$services_page = '';//pixtheme_get_setting('pix-page-team-cat', '');
$all_services = '';
if ( '' != $services_page ) {
...
$image_size = $image_class = pixtheme_get_setting('pix-images-team-cat', '');
if($pix_col == 3){
    $image_size .= '-col-3';
} elseif($pix_col > 3) {
    $image_size .= '-col-4';
...
<!-- TAXONOMY - CONTENT -->
<!-- ========================== -->
<section class="blog pix-taxonomy pix-team-cat">
    <div class="container <?php echo esc_attr($pix_width_class)?>">
        <div class="row">
			...
			<div class="<?php if ( $pix_layout['layout'] != 1 ) : ?>col-xl-9 col-lg-8<?php endif; ?> col-12 <?php echo esc_attr($pix_layout['class']); ?>">
				...
				<div id="team-category-section" class="pix-col-<?php echo esc_attr($pix_col); ?>">

				<?php $pix_cat_description = get_term_field( 'description', $pix_term->term_id, 'pix-team-cat' );
				if( !is_wp_error( $pix_cat_description ) && $pix_cat_description != '' ) :
				?>
					<div class="section-heading text-center">
						<div class="section-subtitle"><?php echo wp_kses_post($pix_cat_description);?></div>
					</div>

				<?php
				endif;

                $pix_cats = get_objects_in_term( $pix_term->term_id, 'pix-team-cat');
                        ...
                        if( $style == 'hover-info' ){
                            $out_content = '
                                <div class="pix-hover-container">
                                    ...
                                    <p>' . wp_kses_post($pix_linked_list_cats) . '</p>
                                </div>
                ...
                $cat_meta = get_option("pix-team-cat_$t_slug");
