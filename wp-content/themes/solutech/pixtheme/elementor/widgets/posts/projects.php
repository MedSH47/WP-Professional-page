...
class Widget_Projects extends Base_Posts {

    public function get_name() {
        return 'pixtheme-projects';
    }

    public function get_title() {
        return esc_html__( 'Projects', 'solutech' );
    }
    ...
    private function add_section_content() {
        ...
        $this->add_control(
            ...
            [
                ...
                'default' => esc_html__( 'more projects', 'solutech' ),
                'type' => Controls_Manager::TEXT,
                'condition' => [
                    'show_button' => 'yes',
                ],
        ...
        $this->add_control(
            ...
            [
                ...
                'description' => esc_html__( 'Leave empty to use all projects link', 'solutech' ),
        ...
        <div class="widget-projects-isotope">
            <div class="projects__top">
                ...
                <?php if ( ! empty( $settings['show_filter'] ) && 'yes' === $settings['show_filter'] ) : ?>
                    ...
                    $terms = [];
                    $attributes = [
                        'taxonomy' => 'pix-portfolio-category',
                    ];
                    $term_ids = [];
                    if ( ! empty( $term_slugs = $settings['include_term_slugs'] ) ) :
                        foreach( $term_slugs as $slug ) {
                            $term = get_term_by( 'slug', $slug, 'pix-portfolio-category' );
            ...
            <?php if ( $posts_query->have_posts() ) : ?>
                ...
                <?php if ( ! empty( $settings['show_button'] ) && 'yes' === $settings['show_button'] && ! empty( $btn_text = $settings['btn_text'] )  ) : ?>
                    <?php
                    $solutech_all_projects_id = solutech_get_option( 'archive_portfolio_page', 'default' );
                    ?>
                    <div class="projects__btn">
                        <?php if ( ! empty( $btn_link = $settings['btn_link']['url'] ) ) : ?>
                            ...
                            <?php else : ?>
                                <?php if ( $solutech_all_projects_id && $solutech_all_projects_id != 'default' ) :
                                    if ( function_exists( 'icl_object_id' ) ) :
                                        $solutech_all_projects_id = icl_object_id( $solutech_all_projects_id, 'page', true );
                                    endif; ?>
                                    <a class="btn btn-black" href="<?php echo esc_url( get_page_link( $solutech_all_projects_id ) ); ?>"><?php echo esc_html( $btn_text ); ?><i class="pix-icon-caret-right"></i></a>
                                <?php else : ?>
                                    <a class="btn btn-black" href="<?php echo get_post_type_archive_link( 'pix-portfolio' ); ?>"><?php echo esc_html( $btn_text ); ?><i class="pix-icon-caret-right"></i></a>
                                <?php endif; ?>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
